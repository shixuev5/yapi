...
class exportController extends baseController {
  ...
  async exportData(ctx) {
    ...
    function createHtml5(left, tp) {
      ...
      <body>
        <div class="m-header">
          ...
          <div class="nav">
            <a href="http://localhost:3000/doc/index.html">YApi</a>
          </div>
        </div>
        <div class="g-doc">
          ${left}
    ...
    function createReqHeaders(req_headers) {
      ...
      if (req_headers && req_headers.length) {
        ...
        for (let j = 0; j < req_headers.length; j++) {
          headersTable += `| ${replaceBr(req_headers[j].name || "") || ""}  |  ${replaceBr(
            req_headers[j].value || ""
          ) || ""} | ${req_headers[j].required ? "是" : "否"}  |  ${replaceBr(
            req_headers[j].example || ""
          ) || ""} |  ${replaceBr(req_headers[j].desc || "") || ""} |\n`;
        }
        return headersTable;
      }
      return "";
    ...
    function createPathParams(req_params) {
      if (req_params && req_params.length) {
        ...
        for (let j = 0; j < req_params.length; j++) {
          paramsTable += `| ${replaceBr(req_params[j].name || "") || ""} |  ${replaceBr(
            req_params[j].example || ""
          ) || ""} |  ${replaceBr(req_params[j].desc || "") || ""} |\n`;
        }
        return paramsTable;
      }
      return "";
    ...
    function createReqQuery(req_query) {
      if (req_query && req_query.length) {
        ...
        for (let j = 0; j < req_query.length; j++) {
          headersTable += `| ${replaceBr(req_query[j].name || "") || ""} | ${
            req_query[j].required ? "是" : "否"
          }  |  ${replaceBr(req_query[j].example || "") || ""} |  ${replaceBr(
            req_query[j].desc || ""
          ) || ""} |\n`;
        }
        return headersTable;
    ...
    function createReqBody(req_body_type, req_body_form, req_body_other) {
      if (req_body_type === "form" && req_body_form.length) {
        ...
        for (let j = 0; j < req_body.length; j++) {
          bodyTable += `| ${replaceBr(req_body[j].name || "") || ""} | ${req_body[j].type ||
            ""}  |  ${req_body[j].required ? "是" : "否"} |  ${replaceBr(
            req_body[j].example || ""
          ) || ""}  |  ${replaceBr(req_body[j].desc || "") || ""} |\n`;
        }
        return `${bodyTable}\n\n`;
    ...
    async function createMarkdown(pid, isToc) {
      //拼接markdown
      //模板
      let mdTemplate = ``;
      const toc = `[TOC]\n\n`;
      try {
        ...
        for (let i = 0; i < interList.length; i++) {
          ...
          mdTemplate += createReqBody(
            interList[i].req_body_type,
            interList[i].req_body_form,
            interList[i].req_body_other
          );
          // Response
          // Response-body
